                    ...
                    try {
                        ...
                        if (data.uid === undefined) {
                            if (data.token !== undefined) {
                                let userFound = false;
                                for (const user in this.data.tokens)
                                    if (this.data.tokens[user] === data.token) {
                                        userFound = true;
                                        ...
                                        try {
                                            const discordUser = yield guild.members.fetch(user);
                                ...
                                for (const user in this.data.relogTokens)
                                    if (this.data.relogTokens[user] === data.token) {
                                        userFound = true;
                            ...
                            if (data.eType === 'ping uid')
                                ws.send(JSON.stringify({ uid: data.uid, eType: 'pong uid' }));
                            if (data.eType === 'click') {
                            ...
                            if (data.eType === 'setSolvesPerAverage') {
                                if (isAdmin)
                                    competition.setSolvesPerAverage(data.number);
                            }
                            ...
                            if (data.eType === 'newComp') {
                                if (isAdmin)
                                    competition.newComp(data.name);
                            }
                            if (data.eType === 'loadComp') {
                                if (data.id === undefined) {
                                    if (isAdmin)
                                        ws.send(JSON.stringify({ uid: data.uid, eType: 'loadComp', names: competition.getSavedCompNames() }));
                                }
                                else {
                                    if (isAdmin)
                                        competition.loadById(data.id);
                                }
                            }
                            ...
                            ws.send(JSON.stringify({ message: "Token expired, please try again.<br><br><button onclick='window.location.href=\"/\"'><h1>Home</h1></button>" }));
                    }
                    catch (e) {
                        console.log('error: ' + e);
                        throw e;
    ...
    cleanSocketList() {
        ...
        for (const controlSocketIndex in this.sockets['control']) {
            if (this.sockets['control'][controlSocketIndex].readyState === 3)
                ...
                this.data.socketCount['control']++;
        }
        for (const socketIndex in this.sockets['action']) {
            if (this.sockets['action'][socketIndex]['socket'].readyState === 3)
