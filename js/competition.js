...
class Competition {
    constructor() {
        ...
        if (fs_1.default.existsSync("competitions/default.json")) {
            this.comp = JSON.parse(fs_1.default.readFileSync("competitions/default.json").toString());
        }
        else {
    ...
    makeDiscordClient() {
        this.discordClient = new discordClient_1.default(this.comp.discordData);
        this.discordClient.onDataChanged = this.saveDiscordData.bind(this);
        return this.discordClient;
    }
    makeWebServer(discordClient) {
        this.webServer = new server_1.default(this, discordClient, this.comp.serverData);
    ...
    save() {
        if (this.comp.competitionId !== undefined) {
            ...
            if (!fs_1.default.existsSync(path)) {
                let directory = JSON.parse(fs_1.default.readFileSync("competitions/directory.json").toString());
                directory[this.comp.competitionId] = this.comp.competitionName;
                fs_1.default.writeFileSync("competitions/directory.json", JSON.stringify(directory));
            }
            fs_1.default.writeFileSync(path, JSON.stringify(this.comp));
        }
        fs_1.default.writeFileSync("competitions/default.json", JSON.stringify(this.comp));
    }
    ...
    loadById(id) {
        ...
        if (!fs_1.default.existsSync(path))
            return false;
        this.comp = JSON.parse(fs_1.default.readFileSync(path).toString());
        if (this.webServer !== undefined)
            this.webServer.resetWith(this.comp.serverData);
        return true;
    }
    loadByName(name) {
        const directory = JSON.parse(fs_1.default.readFileSync("competitions/directory.json").toString());
        let matches = [];
        for (const index in directory)
            if (directory[index] === name)
                matches.push(index);
        if (matches.length === 0)
            return false;
        if (matches.length === 1)
    ...
    regenerateActions() {
        ...
        for (const solveIndex in this.comp.solves) {
            if (this.comp.solves[solveIndex].status === this.STATUS.SOLVE.AWAITING_SCRAMBLE)
                possibleVolunteerSolves['scramble'].push(solveIndex);
            if (this.comp.solves[solveIndex].status === this.STATUS.SOLVE.AWAITING_RUNNER)
                possibleVolunteerSolves['run'].push(solveIndex);
            if (this.comp.solves[solveIndex].status === this.STATUS.SOLVE.AWAITING_JUDGE)
                possibleVolunteerSolves['judge'].push(solveIndex);
        }
            ...
            while (repeat) {
                ...
                if (person.status === this.STATUS.DEAD) {
                    ...
                    if (person.status === this.STATUS.ROLE_SELECT) {
                        if (this.comp.settings.roleSelect.length === 1 && this.comp.settings.roleSelect[0] === 'organizer_chosen') {
                            ...
                            for (const optionIndex in this.comp.settings.roleSelect) {
                                ...
                                if (option === 'scramble')
                                    buttons.push({ text: "Scrambler", value: "scramble" });
                                if (option === 'run')
                                    buttons.push({ text: "Runner", value: "run" });
                                if (option === 'judge')
                                    buttons.push({ text: "Judge", value: "judge" });
                                if (option === 'compete')
                                    buttons.push({ text: "Compete", value: "compete" });
                                if (option === 'self_serve')
                                    buttons.push({ text: "Self serve", value: "self_serve" });
                                if (option === 'organizer_chosen')
                                    buttons.push({ text: "Organizer choice", value: "organizer_chosen" });
                            }
                    ...
                    if (person.status === this.STATUS.COMPETE.CHOOSE_CUP) {
                        if (this.comp.settings.cupNumbers === 'chosen') {
                            ...
                            for (const cupId in this.comp.cups) {
                                ...
                                for (const solveId in this.comp.solves)
                                    if (this.comp.solves[solveId].status !== this.STATUS.SOLVE.COMPLETE
                            ...
                            if (buttons.length === 0)
                                message = "Waiting for organizer to add cups...";
                            ...
                            if (this.comp.settings.autoCupSelect === 'true') {
                                ...
                                for (const cupIndex in this.comp["cups"]) {
                                    ...
                                    let cupTaken = false;
                                    for (const solveIndex in this.comp["solves"])
                                        if (this.comp["solves"][solveIndex].status !== 'complete'
                        ...
                        let event = "three";
                        for (const solveId in this.comp.solves)
                            if (this.comp.solves[solveId].competitor === uid)
                                event = this.comp.solves[solveId].event;
                        this.comp.people[uid].action = {
                            type: "viewAndButtons",
                    ...
                    if (this.comp.solves[person.solve].time === undefined)
                        message += "a judging station.";
                    else
                        message += "the scrambling station.";
                    this.comp.people[uid].action = {
                        type: "buttons",
                        message: message,
                        buttons: [
        ...
        if (person.type === undefined) {
            ...
            if (person.status === this.STATUS.COMPETE.CHOOSE_CUP) {
                ...
                if (value.value === "yes") {
                    ...
                    let partnerId = null;
                    for (const judgeId in this.comp.people)
                        if (this.comp.people[judgeId].status === this.STATUS.JUDGE.AWAITING_CONFIRMATION
                            && this.comp.people[judgeId].solve.toString() === person.solve.toString()) {
    ...
    newComp(name) {
        ...
        let highestNumber = -1;
        for (const index in directory)
            if (parseInt(index) > highestNumber)
                highestNumber = parseInt(index);
        this.comp = {
    ...
    getSavedCompNames() {
        return JSON.parse(fs_1.default.readFileSync("competitions/directory.json").toString());
    }
    ...
    choosePersonCup(uid, cup) {
        if (this.comp.people[uid].type === 'compete')
            this.choosePersonCupHelper(uid, cup, "three");
    }
    ...
    newCup(name) {
        let maxCupId = 0;
        for (const cupId in this.comp.cups)
            if (parseInt(cupId) > maxCupId)
                maxCupId = parseInt(cupId);
    ...
    getSolveResults(id) {
        try {
            if (this.comp.solves[parseInt(id)] && this.comp.solves[id].status === this.STATUS.SOLVE.COMPLETE)
                return this.comp.solves[id];
            else
                return {};
        }
        catch (e) {
    ...
    getPersonAverage(uid, event) {
        ...
        if (sum !== 0) {
            ...
            if (counter < 1 + this.comp.solvesPerAverage) {
                ...
                if (dnfCount === 1)
                    average = ((sum - min) / (counter - 3)).toFixed(2);
                else if (dnfCount >= 2) {
                    if (this.comp.people[uid].type !== 'self_serve')
                        average = 'DNF';
                    else
                        average = ((sum - min) / (counter - 3 - (dnfCount - 1))).toFixed(2);
                }
    ...
    changeSolvePenalty(id, penalty) {
        if (penalty === 'none' || penalty === '+2' || penalty === 'dnf')
            this.comp.solves[id].penalty = penalty;
    }
}
